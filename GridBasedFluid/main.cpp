...
#include "3rdParty/stb-master/stb_image_write.h"
...
// The model here is that there is a drop of ink fall into a container filled with 
// a kind of fluid. We can get the animation of the whole process.
...
public:
	...
	/* Convert fluid density to RGB image */
...
void main()
{
	...
	unsigned char *image = new unsigned char[sizeX * sizeY * 3];
	FluidSolver mSolver = FluidSolver(sizeX, sizeY, density);
	double time = 0.0;
	int iterations = 0;
	...
	while (time < 8.0) {
		for (int i = 0; i < 4; i++)
		{
			mSolver.addInflow(190, 1.0, 20, 5, 1.0, 0.0, 3.0);
			mSolver.update(timestep);
			time += timestep;
		}
		mSolver.toImage(image);
		string name = string("Frame") + to_string(iterations) + string(".png");
		string path = "/result/" + name;
		stbi_write_png(path.c_str(), sizeX, sizeY, 3, image, 0);
	}
	*/
	const int sizeX = 128;
	const int sizeY = 128;
	...
	while (time < 8.0) {
		...
		string name = to_string(iterations) + string(".png");
		if (iterations < 10)
		{
			name = "00" + name;
		}
